...
/** @typedef {import('./resolve.js').FS["readFileSync"]} readFileSync */

/**
 * @param {string} jsonPath
 * @param {readFileSync} readFileSync
 * @param {{specifier: URL | string, base?: URL}=} context
 * @returns {PackageConfig}
 */
export function read(jsonPath, readFileSync, context) {
  ...
  try {
    string = readFileSync(path.toNamespacedPath(jsonPath), 'utf8')
  } catch (error) {
  ...
  if (string !== undefined) {
    ...
    try {
      ...
      const error = new ERR_INVALID_PACKAGE_CONFIG(
        jsonPath,
        context
          ? `"${context.specifier}" from ` +
            fileURLToPath(context.base || context.specifier)
          : '',
        cause.message
      )
      error.cause = cause
 ...
 * @returns {PackageConfig}
 */
export function getPackageScopeConfig(resolved, readFileSync) {
  ...
  while (true) {
    ...
    const packageConfig = read(fileURLToPath(packageJSONUrl), readFileSync, {
      specifier: resolved
    })
 ...
 * @param {{url: URL, readFileSync: readFileSync}} arguments_
 * @returns {PackageType}
 */
export function getPackageType({url, readFileSync}) {
  // To do @anonrig: Write a C++ function that returns only "type".
  return getPackageScopeConfig(url, readFileSync).type
}
