import {format, inspect} from './node-util.js'
...
codes.ERR_INVALID_FILE_URL_HOST = createError(
  'ERR_INVALID_FILE_URL_HOST',
  'File URL host must be "localhost" or empty',
  TypeError
)

codes.ERR_INVALID_FILE_URL_PATH = createError(
  'ERR_INVALID_FILE_URL_PATH',
  'File URL path %s',
  TypeError
)

codes.ERR_INVALID_URL_SCHEME = createError(
...
/**
 * @param {string} warning_
 * @param {string=} type
 * @param {string=} code
 */
function createWarningObject(warning_, type, code) {
  assert(typeof warning_ === 'string')
  /** @type {Error & {code?: string}} */
  const warning = new Error(warning_)
...
function process() {
  if (processMemo !== null) return processMemo
  processMemo = /** @type {Record<string, Record<string, any>>} */ (
...
/** @type {null | boolean} */
let memoNoDeprecation = null
export function noDeprecation() {
  if (memoNoDeprecation !== null) return memoNoDeprecation
  memoNoDeprecation = Boolean(process()?.noDeprecation)
  return memoNoDeprecation
}
...
export function emitWarning(warning, type, code) {
  ...
  if (typeof nativeEmitWarning === 'function') {
    nativeEmitWarning(warning, type, code)
    return
  }
  ...
  if (typeof warning === 'string') {
    warning = createWarningObject(warning, type, code)
